...
// Fan-out: a wing member posted to the feed → notify the other members who opted in.
export async function POST(req: NextRequest) {
  try {
    const { wingId, authorId, authorName, preview } = await req.json();
    ...
    const wrote = authorGender === "female" ? "כתבה בפיד" : "כתב בפיד";

    const body = typeof preview === "string" && preview.trim()
    ...
    await Promise.allSettled(
      recipients.map(async (uid) => {
        ...
        if (!token || data?.notificationPrefs?.feed === false) return;

        try {
          await admin.messaging().send({
            token,
            notification: { title: `📝 ${authorName} ${wrote}`, body },
            data: { link, type: "feed_post" },
            webpush: {
    ...
    console.error("Feed-post notification error:", err);
